...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 18 18:48:01 2023
...
def train_model(args, train_dataset, valid_dataset):
    ...
    model = create_and_compile_model(input_shape, learning_rate=args.initial_learning_rate)  # Set an initial learning rate
...
def test_model(args, test_dataset):
    ...
    # Create and compile the model
    model = create_and_compile_model(input_shape,learning_rate=args.initial_learning_rate)
...
def main():
    ...
    with strategy.scope():
        ...
        train_dataset = CNNDataGenerator(df=train, path2=os.path.join(args.data_dir, "sinograms_3W_train"), shape=(256, 256), batch_size=args.batch_size, num_classes=1, shuffle=True)
        valid_dataset = CNNDataGenerator(df=valid, path2=os.path.join(args.data_dir, "sinograms_3W_train"), shape=(256, 256), batch_size=args.batch_size, num_classes=1, shuffle=True)
        test_dataset = CNNDataGenerator(df=test_yn, path2=os.path.join(args.data_dir, "sinograms_3W_train"), shape=(256, 256), batch_size=args.batch_size, num_classes=1, shuffle=False)
